...
import pydub
import telegram
from synthica_utils import welcome_message, show_friend_profile
...
⚪ /friends – Select AI Friends ({config.chat_mode_count} friends available)
⚪ /balance – Show balance
⚪ /help – Show help

Send me a message and get to know me! I can also answer with audio messages if you send me one.
Learn more at https://synthica.ai/ai-friends
"""
...
async def message_handle(update: Update, context: CallbackContext, message=None, use_new_dialog_timeout=True):
    ...
    if config.chat_modes[chat_mode].get("occupation") == "Artist":
    ...
    async def message_handle_fn():
        ...
        if use_new_dialog_timeout:
            if (datetime.now() - db.get_user_attribute(user_id, "last_interaction")).seconds > config.new_dialog_timeout and len(db.get_dialog_messages(user_id)) > 0:
                db.start_new_dialog(user_id)
                await update.message.reply_text(f"Starting new dialog due to timeout (<b>{config.chat_modes[chat_mode].get('name')}</b>) ✅", parse_mode=ParseMode.HTML)
        db.set_user_attribute(user_id, "last_interaction", datetime.now())
...
async def new_dialog_handle(update: Update, context: CallbackContext):
    ...
    chat_mode = db.get_user_attribute(user_id, "current_chat_mode")
    await update.message.reply_text(f"{welcome_message(config.chat_modes[chat_mode])}", parse_mode=ParseMode.HTML)
...
def get_chat_mode_menu(page_index: int):
    ...
    text = f"Select <b>AI Friends</b> ({config.chat_mode_count} friends available):"
    ...
    for chat_mode_key in page_chat_mode_keys:
        ...
        name += ", " + str(config.chat_modes[chat_mode_key]["age"])
...
async def set_chat_mode_handle(update: Update, context: CallbackContext):
    ...
    image_url, caption = show_friend_profile(config.chat_modes[chat_mode])
...
async def post_init(application: Application):
    await application.bot.set_my_commands([
        ...
        BotCommand("/friends", "Friends List"),
...
def run_bot() -> None:
    ...
    application.add_handler(CommandHandler(
        "friends", show_chat_modes_handle, filters=user_filter))
