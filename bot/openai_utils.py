...
class ChatGPT:
    def __init__(self, model="gpt-3.5-turbo"):
        assert model in {"text-davinci-003", "gpt-3.5-turbo",
                         "gpt-4"}, f"Unknown model: {model}"
        self.model = model

    async def send_message(self, message, dialog_messages=[], chat_mode=list(config.chat_modes.keys())[0]):
        if chat_mode not in config.chat_modes.keys():
            raise ValueError(f"Invalid Friend")
    ...
    async def send_message_stream(self, message, dialog_messages=[], chat_mode=list(config.chat_modes.keys())[0]):
        if chat_mode not in config.chat_modes.keys():
            raise ValueError(f"Invalid Friend")
            ...
            try:
                if self.model in {"gpt-3.5-turbo", "gpt-4"}:
                    ...
                    async for r_item in r_gen:
                        ...
                        n_input_tokens, n_output_tokens = self._count_tokens_from_prompt(
                            prompt, answer, model=self.model)
                        n_first_dialog_messages_removed = n_dialog_messages_before - \
                            len(dialog_messages)
                        yield "not_finished", answer, (n_input_tokens, n_output_tokens), n_first_dialog_messages_removed
    ...
    def _generate_prompt(self, message, dialog_messages, chat_mode):
        prompt = config.chat_modes[chat_mode].get('site_profile')
        prompt += "\n\n"
